...
score_thresholds = [x*0.01 for x in range(1, 11, 1)]

def eval(model, path, detections_file, resize, max_size, batch_size, mixed_precision=True, is_master=True, world=0,
    ...
    results = [[] for i in range(len(score_thresholds))]
    profiler = Profiler(['infer', 'fw'])
    with torch.no_grad():
        for i, (data, ids, ratios) in enumerate(tqdm(data_iterator)):
            # Forward pass
            profiler.start('fw')
            cls_heads, box_heads = model(data, rotated_bbox)
            
            # post-processing
            for i, score_threshold in enumerate(score_thresholds):
                scores, boxes, classes = model.post_processing(data, cls_heads, box_heads, score_threshold)
                results[i].append([scores, boxes, classes, ids, ratios])
    ...
    for i in range(len(score_thresholds)):
        results[i] = [torch.cat(r, dim=0) for r in zip(*(results[i]))]
    ...
    if is_master:
        score_results = [[] for i in range(len(score_thresholds))]
        
        # Copy buffers back to host
        for i in range(len(score_thresholds)):
            score_results[i] = [r.cpu() for r in results[i]]
                
        for score_index, results in enumerate(score_results):
            score_threshold = score_thresholds[score_index]
            ...
            if detections:
                # Save detections
                det_name, ext = detections_file.rsplit('.', 1)
                det_file = '{}_{:.2f}.{}'.format(det_name, score_threshold, ext)
                
                if detections_file and verbose: print('Writing {}...'.format(det_file))
                ...
                if detections_file:
                    json.dump(detections, open(det_file, 'w'), indent=4)
